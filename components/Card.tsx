  ...
  return (
    ...
    >
      <div className={`relative w-full h-full transition-transform duration-[1500ms] transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className={`absolute w-full h-full backface-hidden rotate-y-180 bg-zinc-900 border-4 ${borderColor} rounded-lg shadow-2xl flex flex-col items-center justify-between p-6 overflow-hidden`}>
           {/* Decorative background logo */}
           <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none">
                <RoleIcon role={role} className="w-64 h-64" />
           </div>
           ...
           <div className="mt-8 text-center z-10">
             <div className={`flex justify-center mb-6 drop-shadow-lg ${textColor}`}>
                <RoleIcon role={role} className="w-20 h-20" />
             </div>
           ...
           <p className="text-center font-typewriter text-zinc-400 text-sm leading-relaxed px-2 z-10">
             {roleDesc}
           </p>

           <div className="font-cinzel text-xs text-zinc-700 tracking-[0.3em] z-10">CONFIDENTIAL</div>
        </div>
